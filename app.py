...
def create_app() -> Flask:
    ...
    def usuarios_list():
        ...
        if current_user.is_global_admin():
            usuarios = db.fetchall("SELECT u.id, u.username, u.nombre, u.role, u.activo, c.nombre AS condominio_nombre, u.condominio_id FROM usuarios u LEFT JOIN condominios c ON c.id = u.condominio_id ORDER BY COALESCE(c.nombre, 'ZZZ'), u.nombre, u.username")
        else:
            usuarios = db.fetchall("SELECT u.id, u.username, u.nombre, u.role, u.activo, c.nombre AS condominio_nombre, u.condominio_id FROM usuarios u LEFT JOIN condominios c ON c.id = u.condominio_id WHERE u.condominio_id = ? ORDER BY u.nombre, u.username", (current_user.condominio_id,))
        ...
        if request.method == 'POST':
            ...
            if not current_user.is_global_admin():
                ...
                if role == 'admin' and condominio_id is None:
                    ...
                    return render_template('usuarios_form.html', usuario=None, roles=ALLOWED_ROLES, condominios=condominios)
            if not nombre or not username or not password:
                ...
                db.execute(
                    'INSERT INTO usuarios (username, password_hash, role, nombre, activo, condominio_id) VALUES (?, ?, ?, ?, ?, ?)',
                    (username, generate_password_hash(password), role, nombre, activo, condominio_id)
                )
                db.commit()
    ...
    def usuarios_edit(user_id: int):
        db = get_db()
        usuario = db.fetchone('SELECT id, username, role, nombre, activo, condominio_id FROM usuarios WHERE id=?', (user_id,))
        if not usuario:
        ...
        if request.method == 'POST':
            ...
            if not nombre or not username:
                ...
                if password:
                    db.execute('UPDATE usuarios SET nombre=?, username=?, role=?, activo=?, condominio_id=?, password_hash=? WHERE id=?',
                               (nombre, username, role, activo, condominio_id, generate_password_hash(password), user_id))
                else:
                    db.execute('UPDATE usuarios SET nombre=?, username=?, role=?, activo=?, condominio_id=? WHERE id=?',
                               (nombre, username, role, activo, condominio_id, user_id))
                db.commit()
                flash('Usuario actualizado.', 'success')
                return redirect(url_for('usuarios_list'))
        return render_template('usuarios_form.html', usuario=usuario, roles=ALLOWED_ROLES, condominios=condominios)
